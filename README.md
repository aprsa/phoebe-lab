...
- **Frontend**: Single-page NiceGUI application (`lab/phoebe_ui.py`) served on the configured UI port (default 80)
...
The UI will be available at: **<http://localhost:80>** (or the configured UI port)

## Configuration

Configuration is read from (in order):

- **Editable install** (`pip install -e`): `config.toml` in the project root
- **venv install** (`pip install`): `~/.config/phoebe-lab/config.toml`
- **System-wide install** or **Docker**: `/etc/phoebe-lab/config.toml`

If the config file is missing, built-in defaults are used (localhost:8001 for server, no password).

For Docker deployments, mount your config and examples as volumes:

```yaml
volumes:
  - ./config.toml:/etc/phoebe-lab/config.toml:ro
  - ./examples:/usr/share/phoebe-lab/examples:ro
```

For venv installs, create the config and examples directories:

```bash
mkdir -p ~/.config/phoebe-lab
cp config.toml ~/.config/phoebe-lab/config.toml
mkdir -p ~/.local/share/phoebe-lab/examples
cp -r examples/* ~/.local/share/phoebe-lab/examples/
```
...
Configuration is loaded from `config.toml` in the project root by default, with fallbacks to:

- `~/.config/phoebe-lab/config.toml`
- `~/.phoebe-lab/config.toml`

You can override the config location explicitly:

```bash
phoebe-lab --config /path/to/config.toml
```

The storage secret for session persistence is set in the `ui.run()` call.
...
- **Long-Running Operations**: Execute with `await run.io_bound(client_method, *args)` and show button loading indicators
